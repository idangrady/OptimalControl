...
%Mv...      constant
  ...
  for n = 1:length(si)  %go through given path
    diff = abs(si(n) - s_normed_current*deltas)
...
%            C1_tmp = ((s_kp1-s_normed_current-v_normed_current)*deltas - (v_kp1-v_normed_current)*tau/3*deltav)/tau^2*6;
      ...
      tmp = round(si(n)/deltas);
      for s_kp1 =tmp:-1:1  %try to make as big steps as possible
