...
for i=1:size(idBinary, 1) 
    ...
    wer = P(:, i)==1;
    add_id=MatStateID(P(:, i)==1, :)+MatStateID(i, :);
    ...
    PE_Multiply_01=transpose(r{1}(id_TakeDecision(:,1) + (id_TakeDecision(:,2)-1)*n)).*(add_id(:, [1, end])==0)+transpose(r{2}(id_TakeDecision(:,1)...
        + (id_TakeDecision(:,2)-1)*n)).*(add_id(:, [1, end])==1)+(add_id(:, [1, end])==2);
    
    P_Prod_Check=ones(size(P(P(:, i)==1, i), 1), 1);
    if N>2
        id_TakeDecision=[(MatStateID(i, 1:end-2)+1)', (MatStateID(i, 3:end)+1)']; 
        P_elements=transpose(r{1}(id_TakeDecision(:,1) + (id_TakeDecision(:,2)-1)*n)).*(add_id(:, 2:end-1)==0)+transpose(r{2}(id_TakeDecision(:,1)...
            + (id_TakeDecision(:,2)-1)*n)).*(add_id(:, 2:end-1)==1)+(add_id(:, 2:end-1)==2);
        for k=1:size(P_elements, 2)
            P_Prod_Check=P_Prod_Check.*P_elements(:, k);
        end
    end
    P(P(:, i)==1, i)=P_Prod_Check.*PE_Multiply_01(:, 1).*PE_Multiply_01(:, 2);
end
...
mu_ = policy(2:end-1,:)'+1;
    ...
    % Now, based on the number of state()+ state we know whether its from
    % 0-> 1 and from 1->1.
    wer = P(:, i)==1;
    sum_states=MatStateID(P(:, i)==1, :)+MatStateID(i, :);
    
    sum_cur_states_ = (sum_states(:, [1, end])==1)+(sum_states(:, [1, end])==2);

    % if sum_states ==1, it means 0->1 , if =2 -> from 1->1
    id_TakeDecision=[[1;1], (MatStateID(i, [2, end-1])+1)']; 
    %get multiply 
    ...
    trans_prob_multipl=transpose(r{1}(id_TakeDecision(:,1) + (id_TakeDecision(:,2)-1)*n)).*(id_TakeDecision(:, [1, end])==0)+transpose(r{2}(id_TakeDecision(:,1)...
        + (id_TakeDecision(:,2)-1)*n)).*(id_TakeDecision(:, [1, end])==1)+(id_TakeDecision(:, [1, end])==2);

    P_Prod_Check=ones(size(P(P(:, i)==1, i), 1), 1); % make sure we dont do an illigal transition

    % if N is greater then N, the computation needs to be different
    if N>2
        id_TakeDecision=[(MatStateID(i, 1:end-2)+1)', (MatStateID(i, 3:end)+1)']; 
        P_elements=transpose(r{1}(id_TakeDecision(:,1) + (id_TakeDecision(:,2)-1)*n)).*(sum_states(:, 2:end-1)==0)+transpose(r{2}(id_TakeDecision(:,1)...
            + (id_TakeDecision(:,2)-1)*n)).*(sum_states(:, 2:end-1)==1)+(sum_states(:, 2:end-1)==2);
        for k=1:size(P_elements, 2)
            P_Prod_Check=P_Prod_Check.*P_elements(:, k);
        end
    end
    P(P(:, i)==1, i)=P_Prod_Check.*trans_prob_multipl(:, 1).*trans_prob_multipl(:, 2);
end